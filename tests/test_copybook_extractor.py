...
from src.coboljsonifier.copybookextractor import CopybookExtractor
...
class TestCopybookExtractor(unittest.TestCase):
    ...
    def test_open_book_file_exists(self):
        ...
        ce = CopybookExtractor('tests/test_files/DATA1_BOOK.cob').dict_book_structure
        # print(json.dumps(ce, indent=2))
        self.assertTrue(ce)

    def test_open_book_file_not_exists(self):
        # if file no exists will raise an error
        self.assertRaises(IOError, CopybookExtractor, 'tests/test_files/DATA1_BOOK_NOT_EXISTS.cob')
